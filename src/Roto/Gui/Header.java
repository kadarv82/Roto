...
public class Header {
    ...
    public void createExcelHeader(JTable table, String headerType) {
        ...
        if (headerType.equals(swc.HEADER_OFFER))
            headerData = swc.headerDataOffer;
        ...
        for (int i = 0; i < headerData.length; i++) {
            ...
            if (headerData[i].equals("Date")) {
                Value = dateToday;
            }
            if (headerData[i].equals("PricelistDate")) {
                Value = priceUtil.getPriceListDate();
            }
            if (headerData[i].equals("Total")) {
                Value =
                    swc.formatter.format(database.getTotalPrice()) + " " + priceUtil.getCurrency() + " " +
                    propertyUtil.getLangText("InternalFrame.CollectionList.Vat");
            }
            if (headerData[i].equals("Addressee") && headerType.equals(swc.HEADER_OFFER)) {
                Value = "";
            }
            if (headerData[i].equals("Email") && headerType.equals(swc.HEADER_OFFER)) {
                Value = "";
            }
