...
public class DataBaseTableModel extends DefaultTableModel implements ActionListener, MouseListener{
	...
	public void setProperties(JTable table){
		...
		table.getColumnModel().getColumn(9).setPreferredWidth(155);
	...
	private void setBarComboCell(int column){
		...
		for (int i=0; i < swc.closableBarArray.length; i++){
			barCombo.addItem(propertyUtil.getLangText("InternalFrame.EditDataBase.ClosableBar") + " - " + swc.closableBarArray[i]);
		}
	...
	private Boolean isComboColumn(int column){
		if (comboColumns.contains(column)) return true;
		else return false;
	}
	
	private Boolean isNumberColumn(int column){
		if (numberColumns.contains(column)) return true;
		else return false;
	}
	...
	public void mousePressed(MouseEvent e) {
		if (e.getSource() == table) {
			...
			Frames.getInstance().jbPriceListFilterAdd.setActionCommand("editdatabasesetsap");
		} 
	}
