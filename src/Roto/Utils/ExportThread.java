	...
	private void createDirectories() {
		...
		for (String fileName : fileList){
			...
			if (source.exists()){
				...
				frames.jtExportInfo.setCaretPosition(frames.jtExportInfo.getText().length());
			}
		}
		...
		while (e.hasMoreElements()) {
			...
			//Add users
			if (!selectedUsers.contains(profileUserNode.getTitle())) selectedUsers.add(profileUserNode.getTitle());
			...
			while (en.hasMoreElements()) {
				...
				//Add profiles
				if (!selectedProfiles.contains(profileProfileNode.getTitle())) selectedProfiles.add(profileProfileNode.getTitle());
			}
		}
			...
			for (String user : allUsers){
				...
				for (String profile : allProfiles){
					...
					//Copy directory, if profile exists 
						...
						File sourceDirectory = new File(targetProfileProperties.getProperty(profileDirectoryKey).toString()); 
						File targetDirectory = new File(exportRoot+"\\" + targetProfileProperties.getProperty(profileDirectoryKey).toString());
