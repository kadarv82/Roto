...
public class UserUtil {
    ...
    public void loginUser(String UserName) {
        if (UserName.length() > 0 && !UserName.toLowerCase()
                                              .trim()
            ...
            if (loadUser(UserName).equals("") &&
                message.showConfirmDialog(propertyUtil.getLangText("Confirm.CreateNewUser"))) {
                //Create new user
            ...
            if (!loadUser(UserName).equals("")) {
                ...
                setGUILogin();
                           ...
                           .trim()
                           .equalsIgnoreCase("default")) {
            message.showErrorMessage(propertyUtil.getLangText("Alert.UserName.Default.Message"),
                                     propertyUtil.getLangText("Alert.Input.Error.Title"));
        } else {
            message.showErrorMessage(propertyUtil.getLangText("Alert.UserName.Empty.Message"),
                                     propertyUtil.getLangText("Alert.Input.Error.Title"));
        }
    ...
    public boolean isProfileExists(String profileName) {
        String value =
            propertyUtil.getProfileProperty().getProperty("Profiles." + profileName.toLowerCase() + ".Directory");
    ...
    public void modifyProfileName(String oldProfileName, String newProfileName) {
        if (!isProfileExists(newProfileName)) {
            ...
            //Add new values
            propertyUtil.setProfileProperty("Profiles." + newProfileName + ".User",
                                            profileProperty.getProperty("Profiles." + oldProfileName + ".User")
                                            .toString());
            propertyUtil.setProfileProperty("Profiles." + newProfileName + ".Directory",
                                            profileProperty.getProperty("Profiles." + oldProfileName + ".Directory")
                                            .toString());
    ...
    public void fillProfileCombo(String UserName) {
        String ProfileName = frames.tfProfile
                                   .getText()
                                   .trim();
