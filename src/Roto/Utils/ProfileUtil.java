...
public class ProfileUtil {
    ...
    public void loginProfile(String ProfileName, String UserName) {
        if (ProfileName.length() > 0) {
            if (isUserInProfile(ProfileName, UserName)) {
                ...
                //Load user
                if ((sUserDirectory = getProfileDirectory(ProfileName)).equals("") &&
                    message.showConfirmDialog(propertyUtil.getLangText("Confirm.CreateNewProfile"))) {
                ...
                if (sUserDirectory.length() > 0) {
                    ...
                    if (!fDirectoy.exists()) {
                        fDirectoy.mkdir();
                        message.showErrorMessage(propertyUtil.getLangText("Alert.LoadUserSettings.MissingDirectory"),
                                                 propertyUtil.getLangText("Alert.System.Error.Title"));
                    }
            ...
            //Show message if profile name belongs to another user
                ...
                message.showErrorMessage(propertyUtil.getLangText("Alert.ProfileName.DifferentUser.Message"),
                                         propertyUtil.getLangText("Alert.Input.Error.Title"));
            }

        } else {
            message.showErrorMessage(propertyUtil.getLangText("Alert.ProfileName.Empty.Message"),
                                     propertyUtil.getLangText("Alert.Input.Error.Title"));
        }
    ...
    public String getProfileDefaultRebateFilName(String ProfileName) {
        ...
        String path =
            propertyUtil.getStringProperty(propertyUtil.getProfileProperty(),
                                           "Profiles." + ProfileName + ".RebateFile");
        String fileName = "";
        if (path.length() > 0) {
    ...
    public String getProfileDefaultRebatePath(String ProfileName) {
        ...
        return propertyUtil.getStringProperty(propertyUtil.getProfileProperty(),
                                              "Profiles." + ProfileName + ".RebateFile");
    }
    ...
    public String getProfileDirectory(String ProfileName) {
        ...
        return propertyUtil.getStringProperty(propertyUtil.getProfileProperty(),
                                              "Profiles." + ProfileName + ".Directory");
    }
    ...
    private boolean isUserInProfile(String ProfileName, String UserName) {
        ...
        if (profileUser.length() > 0 && UserName.length() > 0 && ProfileName.length() > 0 &&
            !profileUser.equals(UserName.toLowerCase().trim())) {
            return false;
        } else {
            return true;
        }
    ...
    public String createProfile(String ProfilName, String UserName) {
        ...
        try {
            ...
            while (new File(swc.profilePath + ProfilName.replaceAll(" ", "") + "_" + String.valueOf(iDirectory))
                   .isDirectory() && iDirectory < 3000) {
                iDirectory++;
            }
            ...
            if (!fRebateDirectory.exists())
                fRebateDirectory.mkdir();
            ...
            if (fDefaultRebate.exists()) {
                ...
                propertyUtil.setProfileProperty("Profiles." + ProfilName.toLowerCase().trim() + ".RebateFile",
                                                fTargetRebate.getPath());
            }
