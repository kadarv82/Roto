...
public class ExcelUtil {
    ...
    public void saveExcelDataBase(File outPutFile, JTable table) {
        try {
            ...
            int[] DataBaseCellWidths = {
                700, 2220, 3500, 700, 11700, 2220, 2220, 2220, 2220, 700, 1000, 2220, 2220, 2220, 2220, 2220, 2220,
                3260, 3260, 2220, 2220, 2220, 2220, 1680, 2220
            };
            //Create content
        ...
        try {
            ...
            for (int r = 0; r < header.getRowCount(); r++) {
                ...
                for (int c = 0; c < header.getColumnCount(); c++) {
                    ...
                    sheet.addCell(new Label(colIndex, r + headerRowStart, header.getValueAt(r, c).toString(),
                                            cellFormat));
                }
            }
		    ...
		    */
            ...
            for (int r = 0; r < content.getRowCount(); r++) {
                for (int c = 0; c < content.getColumnCount(); c++) {
                    ...
                    if (content.getValueAt(r, c) != null) {
                        if (c == 2) {
                            sheet.addCell(new Label(c + colStart, r + contentRowStart,
                                                    content.getValueAt(r, c).toString(), cellFormat));
                        } else {
                            //Try to add as number
                            try {
                                String value = contentTable.getValueAt(r, c)
                                                           .toString()
                                                           .trim()
                                                           .replace(",", ".");
                                ...
                                sheet.addCell(new Label(c + colStart, r + contentRowStart,
                                                        contentTable.getValueAt(r, c).toString(), cellFormat));
                            }
                        }
                    }
            ...
            if (comment.trim().length() > 0) {
                WritableCellFormat cellFormat = new WritableCellFormat(fontBold);
                sheet.addCell(new Label(colStart, commentRowStart,
                                        propertyUtil.getLangText("InternalFrame.ExcelList.Comment") + ":", cellFormat));
    ...
    public void saveExcelStockList(File excelFile, JTable contentTable, DefaultListModel fileList) {
        try {
            ...
            sheet.addCell(new Label(0, 1, propertyUtil.getLangText("InternalFrame.StockList.Excel.DataBaseList") + ":",
                                    cellFormatBold));
            sheet.addCell(new Label(3, 1, propertyUtil.getLangText("InternalFrame.StockList.Excel.Date") + ":",
                                    cellFormatBold));
            ...
            sheet.addCell(new Label(0, fileListStartRow + fileList.size() + 1,
                                    propertyUtil.getLangText("InternalFrame.StockList.Excel.StockList") + ":",
                                    cellFormatBold));

            //Process content column header
            for (int c = 0; c < contentTable.getColumnCount(); c++) {

                String columnTitle = contentTable.getColumnModel()
                                                 .getColumn(c)
                                                 .getHeaderValue()
                                                 .toString();
            ...
            for (int r = 0; r < contentTable.getRowCount(); r++) {
                for (int c = 0; c < contentTable.getColumnCount(); c++) {
                    ...
                    if (contentTable.getValueAt(r, c) != null) {
                        if (c == 2) {
                            sheet.addCell(new Label(c, r + contentStartRow, contentTable.getValueAt(r, c).toString(),
                                                    cellFormat));
                        } else {
                            //Try to add as number
                            try {
                                ...
                                sheet.addCell(new Label(c, r + contentStartRow,
                                                        contentTable.getValueAt(r, c).toString(), cellFormat));
                            }
                        }
                    }
