...
public class DataBaseManager {
	...
	private int garniture=0,width=0,height=0,width2=0,height2=0,correctionHeight=0,correctionWidth=0,
				minWidth=999999, maxWidth=0, minHeight=999999, maxHeight=0;
	...
	private boolean hasMDK,hasMDF,hasMDS,hasMKIPP,hasMDK2,hasMDF2,hasMDS2,hasMFREI, hasHandle, hasLiftingMD,
			hasCatchMagnet,hasCatchNormal,hasFan,hasNormalBar,hasClosableBar,hasCilinder,hasSelectionBar;
	...
	public void loadTextDataBase(File DataBaseFile, String loadType){
			...
			try {
				BufferedReader br = new BufferedReader(
					new InputStreamReader(new FileInputStream(DataBaseFile), "UTF8")
				);
				...
				//Skip first row
				if (loadType.equals(swc.LOAD_COLLECTION)) this.type = getTypeFromRow(br.readLine());
				if (loadType.equals(swc.LOAD_EDIT)) this.editingType = getTypeFromRow(br.readLine());
				if (loadType.equals(swc.LOAD_STOCK)) br.readLine();
	...
	public void saveTextDataBase(File outPutFile, JTable table){
		...
		String header = Frames.getInstance().jcbEditDatabaseTyes.getSelectedItem().toString() + 
						swc.dataBaseHeaer;
	...
	private void setExistingItems(boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2, boolean isMDF2, boolean isMDS2, boolean isMFREI,
								  int width, int height){
		//Reset values in normal selection
		//Keep values in double selection
		if (resetHasProperties){
			hasMDK = false;hasMDF = false;hasMDS = false;hasMKIPP = false;hasMDK2 = false;
			hasMDF2 = false;hasMDS2 = false;hasMFREI = false; hasHandle = false; hasLiftingMD = false;
			hasCatchMagnet = false;hasCatchNormal = false;hasFan = false;hasNormalBar = false;
			hasClosableBar = false;hasCilinder=false; hasSelectionBar = false;
			colorList = new ArrayList<String>();
			closableBarList = new ArrayList<String>();
			selectioneBarList = new ArrayList<String>();
		}
		...
		for (int i=0; i<table.getRowCount();i++){
			...
			if (tableRow.isMDK()) hasMDK = true;
			if (tableRow.isMDF()) hasMDF = true;
			if (tableRow.isMDS()) hasMDS = true;
			if (tableRow.isMKIPP()) hasMKIPP = true;
			if (tableRow.isMDK2()) hasMDK2 = true;
			if (tableRow.isMDF2()) hasMDF2 = true;
			if (tableRow.isMDS2()) hasMDS2 = true;
			if (tableRow.isMFREI()) hasMFREI = true;
			...
			if (isMDK && !tableRow.isMDK()) selectRow = false;
			if (isMDF && !tableRow.isMDF()) selectRow = false;
			if (isMDS && !tableRow.isMDS()) selectRow = false;
			if (isMKIPP && !tableRow.isMKIPP()) selectRow = false;
			if (isMDK2 && !tableRow.isMDK2()) selectRow = false;
			if (isMDF2 && !tableRow.isMDF2()) selectRow = false;
			if (isMDS2 && !tableRow.isMDS2()) selectRow = false;
			if (isMFREI && !tableRow.isMFREI()) selectRow = false;
			...
			if (selectRow) {
				...
				if (tableRow.isHandle() || tableRow.isHandleGarniture()) hasHandle = true;
				if (tableRow.isLiftingMD() && matchWidthAndHeight) hasLiftingMD = true;
				if (tableRow.isCatchNormal() && matchWidthAndHeight) hasCatchNormal = true;
				if (tableRow.isCatchMagnet() && matchWidthAndHeight) hasCatchMagnet = true;
				if (tableRow.isFan() && matchWidthAndHeight) hasFan = true;
				if (tableRow.isNormalBar()) hasNormalBar = true;
				...
				if (tableRow.isCilinder() && matchWidthAndHeight) hasCilinder = true;
			}
		}
		
	}
	
	private void setMinMaxValues(TextDBRow row, boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2, boolean isMDF2, boolean isMDS2, boolean isMFREI){
		boolean selectRow = true;
		if (isMDK && !row.isMDK()) selectRow = false;
		if (isMDF && !row.isMDF()) selectRow = false;
		if (isMDS && !row.isMDS()) selectRow = false;
		if (isMKIPP && !row.isMKIPP()) selectRow = false;
		if (isMDK2 && !row.isMDK2()) selectRow = false;
		if (isMDF2 && !row.isMDF2()) selectRow = false;
		if (isMDS2 && !row.isMDS2()) selectRow = false;
		if (isMFREI && !row.isMFREI()) selectRow = false;
		if (selectRow) {
			if (row.getMFFBV() < minWidth) minWidth = row.getMFFBV();
			if (row.getMFFBB() > maxWidth) maxWidth = row.getMFFBB();
			
			if (row.getMFFHV() < minHeight) minHeight = row.getMFFHV();
			if (row.getMFFHB() > maxHeight) maxHeight = row.getMFFHB();
		}
	...
	public void selectCollectionRows(boolean left, boolean right, boolean over, boolean handleSelected, boolean liftingMD, String colorSign, int width, int height,
							boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2, boolean isMDF2, boolean isMDS2, boolean isMFREI,
							boolean isMDSMDF2, boolean isMDSMDK2, boolean catcNoneSelected, boolean catcNormalSelected, boolean catcMagnetSelected, boolean fan, boolean closableBarSelected,
							String closableBarSize, boolean normalBarSelected, boolean cilinderSelected, boolean clearSelection, String selectionBarSign){
		...
		minHeight=999999;
		maxHeight=0;
		String className = WindowSystems.getInstance().getWindowSystemByTypeName(getType()).getSystemName();
		...
		for (int i=0; i<table.getRowCount();i++){
			...
			if (tableRow.getItemCount() < 1) selectRow = false;
			if (width < tableRow.getMFFBV() || width > tableRow.getMFFBB() ) selectRow = false;
			if (height < tableRow.getMFFHV() || height > tableRow.getMFFHB() ) selectRow = false;
			if (left && tableRow.isRight()) selectRow = false;
			if (right && tableRow.isLeft()) selectRow = false;
			if (!handleSelected && (tableRow.isHandle() || tableRow.isHandleGarniture())) selectRow = false;
			if (handleSelected && normalBarSelected && tableRow.isHandleGarniture()) selectRow = false;
			if (handleSelected && closableBarSelected && tableRow.isHandle()) selectRow = false;
			if (!liftingMD && tableRow.isLiftingMD()) selectRow = false;
			if (tableRow.isColor() && !tableRow.getColor().equals(colorSign)) selectRow = false;
			if (isMDK && !tableRow.isMDK()) selectRow = false;
			if (isMDF && !tableRow.isMDF()) selectRow = false;
			if (isMDS && !tableRow.isMDS()) selectRow = false;
			if (isMKIPP && !tableRow.isMKIPP()) selectRow = false;
			...
			if (isMDS2 && !tableRow.isMDS2()) selectRow = false;
			if (isMFREI && !tableRow.isMFREI()) selectRow = false;
			if (catcNoneSelected && (tableRow.isCatchMagnet() || tableRow.isCatchNormal())) selectRow = false;
			if (catcNoneSelected && (tableRow.isPullHandle())) selectRow = false;
			if (catcMagnetSelected && (tableRow.isCatchNormal())) selectRow = false;
			if (catcNormalSelected && (tableRow.isCatchMagnet())) selectRow = false;
			if (!fan && tableRow.isFan()) selectRow = false;
			if (tableRow.isCloasbleBar() && closableBarSelected && !tableRow.getLock().equals(closableBarSize)) selectRow = false;
			if (closableBarSelected && tableRow.isNormalBar()) selectRow = false;
			if (!closableBarSelected && tableRow.isElolapDornos()) selectRow = false;
			if (normalBarSelected && tableRow.isCloasbleBar()) selectRow = false;
			if (tableRow.isSelectionBar() && !tableRow.getSelectionBar().equals(selectionBarSign)) selectRow = false;
	
			if (!cilinderSelected && tableRow.isCilinder()) selectRow = false;
			...
			if (selectRow) {
				...
				if (tableRow.getMSISTK() > 0 && tableRow.getMSISTART().length() > 0){
					addSelectionRow(tableRow.getMSISTART(), (tableRow.getMSISTK() * tableRow.getItemCount() * garniture ));
	...
	public void createDataBaseEdit(JTable jtEditTable, boolean isEmptyTable){
		...
		if (isEmptyTable) tableEdit.removeRows();
		...
		for (int i=0; i<tableEdit.getRowCount();i++){
			...
			dtm.addRow(new Object[]{
					String.valueOf(tableRow.getItemCount()),
					String.valueOf(tableRow.getSAP()),
					String.valueOf(tableRow.getDirection()),
					priceUtil.getListText(String.valueOf(tableRow.getSAP())),
					tableRow.isMDK(),
					tableRow.isMDF(),
					tableRow.isMDS(),
					tableRow.isMKIPP(),
					String.valueOf(tableRow.getLock()),
					String.valueOf(tableRow.getColor()),
					String.valueOf(tableRow.getMSSTK()),
					String.valueOf(tableRow.getMSISTK()),
					String.valueOf(tableRow.getMFFBV()),
					String.valueOf(tableRow.getMFFBB()),
					String.valueOf(tableRow.getMFFHV()),
					String.valueOf(tableRow.getMFFHB()),
					String.valueOf(tableRow.getMSTART()),
					String.valueOf(tableRow.getMSISTART()),
					tableRow.isMDK2(),
					tableRow.isMDF2(),
					tableRow.isMDS2(),
					tableRow.isMFREI()
	...
	public void createCollection(JTable jtCollection) {
		if (this.selection != null) {
			...
			collection.setColorName(propertyUtil.getLangText(propertyUtil.getKeysFromValue(propertyUtil.getColorMap(), swc.getSelectedColorSign())));
			collection.setDirection(direction);
			if (dataBaseName.endsWith(".prn")) dataBaseName = dataBaseName.substring(0,dataBaseName.indexOf(".prn"));
	...
	public void selectStockRows(boolean left, boolean right, boolean over, boolean handle, boolean liftingMD, List<String> colorSigns,
			boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2, boolean isMDF2, boolean isMDS2, boolean isMFREI,
			boolean catcNone, boolean catcNormal, boolean catcMagnet, boolean fan, boolean closableBar,
			List<String> barSizes, boolean normalBar, boolean cilinder, boolean clearStockSelection, List<String> barTypes){
		
		if (clearStockSelection) selection = new HashMap<String, Integer>();
		//Select rows for stock list
		for (int i=0; i<tableStock.getRowCount();i++){
			...
			if (isMDK && tableRow.isMDK()) selectRow = true;
			if (isMDF && tableRow.isMDF()) selectRow = true;
			if (isMDS && tableRow.isMDS()) selectRow = true;
			if (isMKIPP && tableRow.isMKIPP()) selectRow = true;
			if (isMDK2 && tableRow.isMDK2()) selectRow = true;
			if (isMDF2 && tableRow.isMDF2()) selectRow = true;
			if (isMDS2 && tableRow.isMDS2()) selectRow = true;
			if (isMFREI && tableRow.isMFREI()) selectRow = true;
			
			if (tableRow.getItemCount() < 1) selectRow = false;
			if (left && tableRow.isRight()) selectRow = false;
			if (right && tableRow.isLeft()) selectRow = false;
			if (!handle && (tableRow.isHandle() || tableRow.isHandleGarniture())) selectRow = false;
			if (handle && normalBar && tableRow.isHandleGarniture() && !closableBar) selectRow = false;
			if (handle && closableBar && tableRow.isHandle() && !normalBar) selectRow = false;
			if (!liftingMD && tableRow.isLiftingMD()) selectRow = false;
			
			if (tableRow.isColor() && !colorSigns.contains(tableRow.getColor())) selectRow = false;
			
			if (catcNone && (tableRow.isCatchMagnet() || tableRow.isCatchNormal())) selectRow = false;
			if (catcNone && (tableRow.isPullHandle())) selectRow = false;
			if (catcMagnet && (tableRow.isCatchNormal())) selectRow = false;
			if (catcNormal && (tableRow.isCatchMagnet())) selectRow = false;
			if (!fan && tableRow.isFan()) selectRow = false;
	
			if (tableRow.isCloasbleBar() && closableBar && !barSizes.contains(tableRow.getLock())) selectRow = false;
			if (tableRow.isSelectionBar() && !barTypes.contains(tableRow.getSelectionBar())) selectRow = false;

			if (!closableBar && tableRow.isCloasbleBar()) selectRow = false;
			if (!closableBar && tableRow.isElolapDornos()) selectRow = false;
			if (!normalBar && tableRow.isNormalBar()) selectRow = false;
			
			if (!cilinder && tableRow.isCilinder()) selectRow = false;
			//Select rows for stock list
			if (selectRow) {
				//Add selected rows
	...
	public void createStockCollection(JTable table, HashMap<String,Integer> stockSelection){
		if (stockSelection ==null) stockSelection = this.selection;
