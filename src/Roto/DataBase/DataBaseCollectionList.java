...
public class DataBaseCollectionList {
	...
	public void createListItems(JList list){
		...
		for (DataBaseCollection collection : databaseCollectionList){
			...
			if (collection.getTypeName().equals(swc.LIST_SUMMARIZED)){
				listItem = counter + ". " + propertyUtil.getLangText("InternalFrame.CollectionEdit.Item") + ": 1 * " 
	...
	public void createListSummarized(DefaultTableModel tableModelList){
		...
		if (databaseCollectionList.size() == 0) return;
		...
		for (int  i=0; i<key.length; i++)   {
			...
			if (Frames.getInstance().jrPrintMartikel.isSelected()){
			...
			tableModelList.addRow(new Object[]{
					selectionSummarized.get(SAP).toString(), 
					ID,
					priceUtil.getListText(SAP),
					formatter.format(priceUtil.getListPriceMultiplied(SAP)),
					formatter.format(priceUtil.getListPriceCalculated(SAP)),
		...
		if (selectionSummarized.size() >0){
			...
			tableModelList.addRow(new Object[]{
					"", 
					"",
					propertyUtil.getLangText("InternalFrame.CollectionList.Sum") + propertyUtil.getLangText("InternalFrame.CollectionList.Vat") + ":",
					"",
					"",
	...
	private void createList(DefaultTableModel tableModelList, boolean isGarnitureList){
		...
		for (DataBaseCollection collection : databaseCollectionList){
			...
			if (collection.getSelection().size() == 0) return;
			...
			if (!collection.getTypeName().equals(swc.LIST_SUMMARIZED)){
					...
					String description =  collection.getTypeName() + ", " + collection.getDirection()+" ," + collection.getColorName();
					String size2="";
					if (collection.getWidth2() != 0 ) size2 = collection.getWidth2() + "*" + collection.getHeight2();
					
					tableModelList.addRow(new Object[]{
							propertyUtil.getLangText("InternalFrame.CollectionList.Item") + ":", 
							counter+".", 
							collection.getDatabaseName(),
							collection.getWidth() +"*"+ collection.getHeight(),
							size2,
							propertyUtil.getLangText("InternalFrame.CollectionList.Garniture") + ": " + collection.getGarniture()});
			...
			//Add summary
			if (!isGarnitureList) tableModelList.addRow(new Object[]{});
			tableModelList.addRow(new Object[]{
					"", 
					"",
					propertyUtil.getLangText("InternalFrame.CollectionList.Sum") + propertyUtil.getLangText("InternalFrame.CollectionList.Vat") + ":",
	...
	public List<SerializableDataBaseCollection> getSerializedCollectionList(String listType){
		...
		//Create summarized list
			...
			for (DataBaseCollection collection : databaseCollectionList){
				...
				if (!collection.getTypeName().equals(swc.LIST_SUMMARIZED)){
					description +=  " - " + collection.getTypeName() + "," + collection.getDirection()+"," 
					+ collection.getWidth() + "*" + collection.getHeight();
