...
public class WindowSystems {
	...
	public WindowSystem getWindowSystemBySystemName(String systemName){
		...
		for (WindowSystem windowSystem : windowSystemList){
			if (windowSystem.getSystemName().toLowerCase().trim().equals(
					systemName.toLowerCase().trim())){
				system = windowSystem;
			}
		}
		return system;
	...
	public WindowSystem getWindowSystemByTypeName(String typeName){
		WindowSystem system = null;
		for (WindowSystem windowSystem : windowSystemList){
			if (windowSystem.getSystemType().toLowerCase().trim().equals(
	...
	public Boolean isSystemNameExit(String systemName){
		if (getWindowSystemBySystemName(systemName) != null) return true;
		else return false;
	}
	
	public Boolean isSystemTypeExit(String typeName){
		if (getWindowSystemByTypeName(typeName) != null) return true;
		else return false;
	}
	...
	public void clearList(){
		...
		windowSystemList = new ArrayList<WindowSystem>();
		Frames.getInstance().jcbWinowTypes.removeAllItems();
	}
	...
	public void load(String filePath){
		try {
			...
			for (int i = 0; i < windowSystemList.getLength(); i++) {
				...
				for (int j = 0; j < windowSystemItemList.getLength(); j++) {	
					...
					if (windowSystemNode.getNodeName().equals("SystemData")){
						...
						for (int k = 0; k < windowSystemDataList.getLength(); k++) {
							...
							int row = Integer.parseInt(dataNode.getAttributes().getNamedItem("x").getTextContent());
							int col = Integer.parseInt(dataNode.getAttributes().getNamedItem("y").getTextContent());
				...
				Frames.getInstance().jcbWinowTypes.addItem(windowSystem.getSystemType());
