...
public class WindowLimitTableModel extends DefaultTableModel implements ActionListener, PropertyChangeListener {
    ...
    public WindowLimitTableModel() {
        ...
        for (int i = 0; i < 8; i++) {
            addRow(new Object[] { new Boolean(false), swc.windowTypeArray[i], "", "", "", "", "", "", "" });
        }
    ...
    public boolean isCellEditable(int row, int col) {
        ...
        if (col == 0 && table.getValueAt(row, 1)
                             .toString()
                             .length() == 0) {
            editable = false;
        }
    ...
    public String getWarningMessage(boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2,
    ...
    public void propertyChange(PropertyChangeEvent evt) {
        if (evt.getSource() == this.table) {
            ...
            if (selectedColumn == 0 || selectedColumn == 6 || table.getSelectedColumn() == 7) {
    ...
    private void checkTableRow(int row) {
        if (row > -1 && isCheckSelectedValues()) {
            ...
            } else if (table.getEditingColumn() > -1 && table.getEditingColumn() != 6 &&
                       table.getEditingColumn() != 7) {
            ...
            if (table.getValueAt(row, 6)
                     .toString()
                     .length() == 0 && getBooleanValue(table.getValueAt(row, 0))) {
    ...
    public boolean getConditionResult(boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2,
                                      boolean isMDF2, boolean isMDS2, boolean isMFREI) {
        ...
        if (conditionText.equals("=")) {
            result =
                getFirsConditionResult(type) == Double.parseDouble(getValueAt(row, 6).toString().replaceAll(",", "."));
        } else if (conditionText.equals(">")) {
            result =
                getFirsConditionResult(type) > Double.parseDouble(getValueAt(row, 6).toString().replaceAll(",", "."));
        } else if (conditionText.equals("<")) {
    ...
    private String getTypename(boolean isMDK, boolean isMDF, boolean isMDS, boolean isMKIPP, boolean isMDK2,
                               boolean isMDF2, boolean isMDS2, boolean isMFREI) {
        String type = "";
        if (isMDK) {
