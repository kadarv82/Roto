...
public class WindowTypes {
    ...
    private boolean left, right, over, handle, liftingMD, isMDK, isMDF, isMDS, isMKIPP, isMDK2, isMDF2, isMDS2, isMFREI, isMDSMDF2, isMDSMDK2, catcNone, catcNormal, catcMagnet, fan, closableBar, normalBar, cilinder, isDoor;
    ...
    public void setSelection(boolean left, boolean right, boolean over, boolean handle, boolean liftingMD,
                             String colorSign, int width, int height, boolean isMDK, boolean isMDF, boolean isMDS,
                             boolean isMKIPP, boolean isMDK2, boolean isMDF2, boolean isMDS2, boolean isMFREI,
                             boolean isMDSMDF2, boolean isMDSMDK2, boolean catcNone, boolean catcNormal,
                             boolean catcMagnet, boolean fan, boolean closableBar, String closableBarSize,
                             boolean normalBar, boolean cilinder) {
    ...
    //Create double frame
    private void createDoubleFrame(boolean CreateFrameToLeft, boolean componentsEnabled, boolean leftComponents,
                                   int frameDistanceWidth, int frameDistanceHeight) {
        Point origLocation = wComponents.getLocation();
        if (CreateFrameToLeft) {
            wComponents.setLocation(origLocation.x - wComponents.getSize().width / 2,
                                    origLocation.y - frameDistanceHeight);
        } else {
            wComponents.setLocation(origLocation.x + wComponents.getSize().width / 2 + frameDistanceWidth,
                                    origLocation.y - frameDistanceHeight);
        }
    ...
    private void createDoubleFrameArc(boolean CreateFrameToLeft, boolean componentsEnabled, boolean leftComponents) {
        ...
        if (!componentsEnabled) {
            if (left)
                wComponents.drawFrameArc(0.3f, "right");
            if (right)
                wComponents.drawFrameArc(0.3f, "left");
        }
        //Draw components if enabled.
        else {
    ...
    private void createDoubleFrameSegment(boolean CreateFrameToLeft, boolean componentsEnabled,
                                          boolean leftComponents) {
        Point origLocation = wComponents.getLocation();
        if (CreateFrameToLeft) {
            wComponents.setLocation(origLocation.x - wComponents.getSize().width / 2, origLocation.y);
        } else {
        ...
        if (!componentsEnabled) {
            if (left)
                wComponents.drawFrameSegment(0.3f, "right");
            if (right)
                wComponents.drawFrameSegment(0.3f, "left");
        }
        //Draw components if enabled.
        else {
    ...
    private void createHKSFrame(boolean CreateFrameToLeft) {
        ...
        if (CreateFrameToLeft) {
            ...
            wComponents.setLocation(wComponents.getLocation().x + (wComponents.getSize().width + 5) * 3 + 5,
                                    origLocation.y);
            wComponents.drawFrame(0.3f);
            wComponents.drawGlass();
        }
    ...
    private void createPusherFrame(boolean CreateFrameToLeft, boolean drawArrays) {
        ...
        if (CreateFrameToLeft) {
            ...
            wComponents.setRight();
            //Draw the fourth frame from 4 to the right size			
            wComponents.setLocation(wComponents.getLocation().x - 10 + (wComponents.getSize().width + 5) * 3 - 15,
                                    origLocation.y - 8);
            wComponents.drawFrame(0.3f);
            wComponents.drawGlass();
            if (drawArrays)
            ...
            wComponents.setLeft();
            //Draw the first frame from 4 to the left size
            wComponents.setLocation(wComponents.getLocation().x - ((wComponents.getSize().width + 5) * 3) + 25,
                                    origLocation.y - 8);
            wComponents.drawFrame(0.3f);
            wComponents.drawGlass();
            if (drawArrays)
    ...
    public void drawClass_1() {
        ...
        wComponents.drawGlass();
        if (handle)
            wComponents.drawHandle();
        wComponents.drawBar();
        if (cilinder)
            wComponents.drawCilinder();
        wComponents.drawDirection();
        if (liftingMD)
            wComponents.drawLiftingMD();
        if (!catcNone)
            wComponents.drawCatch(catcMagnet);
        if (fan)
            wComponents.drawFan();
        wComponents.drawFrame();
        wComponents.drawStrap();
    ...
    public void drawClass_5() {
        ...
        if (isMDS || isMKIPP || isMDK2 || isMDF2) {
            wComponents.drawLineDashed(3, origLocation.x - wComponents.getSize().width - 5,
                                       origLocation.y + wComponents.getSize().height,
                                       origLocation.x + wComponents.getSize().width * 2 + 5,
                                       origLocation.y + wComponents.getSize().height);
        }
    ...
    public void drawClass_7() {
        ...
        if (isMDS) {
            ...
            if (left) {
                wComponents.setRight();
                wComponents.setLocation(wComponents.getLocation().x + wComponents.getSize().width - 5,
                                        wComponents.getLocation().y);
                wComponents.drawDirectionArraySimple();
            }
            if (right) {
                wComponents.setLeft();
