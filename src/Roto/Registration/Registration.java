...
public class Registration extends JFrame implements ActionListener {
    ...
    void generateKey() {
        String code =
            registerUtil.getCodeByKey((Rk1.getText() + "     ").substring(0, 5),
    ...
    void saveRegistryFile() {
        ...
        if (this.registration != null) {
            ...
            if (fc.showSaveDialog(this) == fc.APPROVE_OPTION) {
                ...
                if (!out.exists() ||
                    (out.exists() &&
                     message.showConfirmDialog("This file is exists ! Are you sure you want to overwrite it?"))) {
    ...
    public Registration() {
        ...
        pnReg.add(Rk1 = new JTextField("", 6));
        pnReg.add(new JLabel("-"));
        pnReg.add(Rk2 = new JTextField("", 6));
        pnReg.add(new JLabel("-"));
        pnReg.add(Rk3 = new JTextField("", 6));
        pnReg2.add(new JLabel("Password:  "));
        pnReg2.add(Rd1 = new JTextField("", 6));
        pnReg2.add(new JLabel("-"));
        pnReg2.add(Rd2 = new JTextField("", 6));
        pnReg2.add(new JLabel("-"));
    ...
    public void actionPerformed(ActionEvent ev) {
        if (ev.getSource() == BtRk || ev.getSource() == btEm) {
            System.exit(0);
        }
        if (ev.getSource() == BtRm) {
            saveRegistryFile();
        }
        if (ev.getSource() == BtRny) {
            openRegistryFile();
        }
        if (ev.getSource() == BtRg) {
            generateKey();
        }
        if (ev.getSource() == btEok || ev.getSource() == jtPassword) {
            login();
        }
    }
